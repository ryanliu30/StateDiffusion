...
class ResidualMLPDenoiser(nn.Module):
    def __init__(
        ...
        if cond_dim is not None:
            self.proj = nn.Linear(d_in + cond_dim, max(dim_t, d_in))
            self.conditional = True
        else:
            self.proj = nn.Linear(d_in, max(dim_t, d_in))
        ...
        self.time_mlp = nn.Sequential(
            sinu_pos_emb,
            nn.Linear(fourier_dim, max(dim_t, d_in)),
            nn.SiLU(),
            nn.Linear(max(dim_t, d_in), max(dim_t, d_in))
        )
